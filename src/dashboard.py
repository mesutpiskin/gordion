...
# Main content
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Overview", "📋 Recent PRs", "📈 Analytics", "🤖 Ollama", "📝 Logs"])
...
with tab4:
    ...
    with col2:
        ...
        # Auto-refresh toggle for metrics
        auto_refresh = st.checkbox("Auto-refresh metrics (every 15s)", value=True)
        ...
        if metrics['process_status'] == 'running':
            ...
            if metrics.get('gpu_utilization') != 'N/A':
                try:
                    gpu_value = float(str(metrics['gpu_utilization']).strip('%'))
...
with tab5:
    ...
    # Auto-refresh toggle
    auto_refresh = st.checkbox("Auto-refresh (every 15s)")
